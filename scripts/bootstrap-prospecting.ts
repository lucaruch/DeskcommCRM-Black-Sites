...
const organizationId =
  orgArg?.slice("--organization-id=".length) ?? process.env.PROSPECTING_ORGANIZATION_ID;
const apply = args.has("--apply");
const createToken = args.has("--token");

if (!organizationId)
  throw new Error("Informe --organization-id=<uuid> ou PROSPECTING_ORGANIZATION_ID.");
...
async function main() {
  ...
  try {
    ...
    const org = await db.query<{ id: string }>("select id from organizations where id=$1", [
      organizationId,
    ]);
    ...
    const initial =
      stages.rows.find((stage) => stage.slug === "novo") ??
      stages.rows.find((stage) => !stage.is_won && !stage.is_lost);
    const sent =
      stages.rows.find((stage) => stage.slug === "em_andamento") ??
      stages.rows.find((stage) => !stage.is_won && !stage.is_lost && stage.id !== initial?.id);
    ...
    if (!reply && apply) {
      const position =
        Math.max(...stages.rows.map((stage) => Number(stage.position) || 0), 0) + 1000;
      ...
      reply = {
        ...
        slug: "prospeccao_respondeu",
        name: "Resposta da prospeccao",
        position,
        is_won: false,
        is_lost: false,
      };
      ...
      if (apply) {
        const updated = await db.query<{ status: string }>(
          `update prospecting_campaigns set channel_session_id=coalesce(channel_session_id,$2),reply_stage_id=$3,
             settings=$4,status=case when status in ('draft','paused') and $2 is not null then 'active' else status end,
             updated_at=now()
           where organization_id=$1 and id=$5 returning status`,
          [organizationId, channelId, reply.id, settings, campaignId],
        );
        ...
        await db.query(
          `insert into prospecting_campaigns
           ...
           values($1,$2,'Prospecção Automática - Black Sites','prospeccao-automatica',
             'Cadencia de prospeccao para leads autorizados.', 'chatgpt_daily_prospecting',
             $3,$4,$5,$6,$7,$8,$9,$10)`,
          [
            campaignId,
            organizationId,
            ["prospeccao", "black-sites"],
            campaignStatus,
            pipelineId,
            initial.id,
            sent.id,
    ...
    if (apply && createToken) {
      const existing = await db.query<{ id: string }>(
        "select id from api_tokens where organization_id=$1 and name='ChatGPT - Prospecção Black Sites' and revoked_at is null limit 1",
        [organizationId],
      );
      if (!existing.rows[0]) {
        const admin = await db.query<{ id: string }>(
          "select user_id as id from user_organizations where organization_id=$1 and role='admin' and revoked_at is null order by created_at asc limit 1",
          [organizationId],
        );
    ...
    process.stdout.write(
      JSON.stringify({
        ...
        channel_configured: Boolean(channelId),
        mode: apply ? "applied" : "dry_run",
        ...(tokenPlaintext ? { token_created: true, token_plaintext: tokenPlaintext } : {}),
      }) + "\n",
    );
  } catch (error) {
    try {
